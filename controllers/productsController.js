...
module.exports={
    ...
    elementById: async function(req,res,next){
        try{
            const elements = await products.findById({_id:req.params.id},req.body)
            res.json(elements);
        }catch(err){
            console.log(err);
        }
    ...
    elementByName: async function(req,res,next){
        try{
            const elementsName = await products.findByName().where("name").equals("name");
    ...
    allElementUp: async function(req,res,next){
        try{
        const upAll = await products.updateAll({name:req.params.name},req.body)
        res.json(upAll)
        }catch(err){
            console.log(err);
        }
